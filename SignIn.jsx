...
const handleSignup = async (e) => {
  ...
  try {
    ...
    if (result.success && role === "attendee") {
      ...
      if (insertError) {
        console.log('Attendee insert error:', insertError);
        Alert.alert('Signed up, but attendee profile couldn’t be saved.');
        return;
      }

      Alert.alert('Successfully signed up and registered!');
      router.navigate('/(forms)/Login');
    } else if (result.success) {
      Alert.alert('Signed up, but not registered as an attendee.');
      router.navigate('/(forms)/Login');
    }
  } catch (error) {
    console.log("There was an error during sign-up:", error);
